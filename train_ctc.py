...
class TrainingThread(Thread):
   ...
   def __init__(self,  progress_notifier = None):
   ...
   def run(self):
      return self.train_main_fun(self.progress_notifier)
   ...
   def get_loader(self, chunk_list, mode):
      ...
      dset = MyDataset()
      if mode == 'train':
            loader = DataLoader(dset, batch_size = 4, shuffle = True, collate_fn = self._collate_fn, num_workers = 8, pin_memory = False) # wychodzi mi, że jedno t to 4 wypowiedz, to ta sama wartość co batch size
      elif mode == 'test':
            loader = DataLoader(dset, batch_size = 4, shuffle = False, collate_fn = self._collate_fn, num_workers = 8, pin_memory = False)
      else:
   ...
   def train_one_epoch(self, model, loss_fn, optimizer, print_every = 10, progress_notifier=None):
      ...
      for i in range(0, train_num_chunks, load_chunks_every):
            ...
            for data in loader_train:
               ...
               if (t + 1) % print_every == 0:
                  ...
                  if self.should_be_stopped(): return
                  if progress_notifier != None:
                     progress_notifier.current_sample = batch_size * t
                     progress_notifier.current_loss = loss.data[0]
   ...
   def check_accuracy(self, model):
      ...
      for i in range(0, val_num_chunks, load_chunks_every):
         chunk_list = data_list[i: i + load_chunks_every]
         loader_val = self.get_loader(chunk_list, 'test')
   ...
   def train_epochs(self, model, loss_fn, init_lr, model_dir, progress_notifier = None):
   ...
   def train_main_fun(self, progress_notifier = None):
      ...
      loss_fn = CTCLoss()
      self.train_epochs(model, loss_fn,  1e-3, 'weights_ctc', progress_notifier=progress_notifier)
